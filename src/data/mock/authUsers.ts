...
function findMockAccount(email: string, password: string): MockAuthAccount | undefined {
    const normalized = email.trim().toLowerCase();
    return MOCK_AUTH_ACCOUNTS.find((account) => account.email === normalized && account.password === password);
}
function createMockToken(email: string): string {
    return `mock:${email.trim().toLowerCase()}`;
}
function saveMockUserSession(email: string, user: User): void {
    try {
...
function getMockUserFromToken(token: string): User | null {
    ...
    try {
        ...
        if (stored?.email === email && stored.user)
            return stored.user;
    }
    catch {
        // ignore
...
function isMockEmailRegistered(email: string): boolean {
    const normalized = email.trim().toLowerCase();
