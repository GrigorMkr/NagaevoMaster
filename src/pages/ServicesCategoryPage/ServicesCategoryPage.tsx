...
function ServicesCategoryPage() {
    ...
    return (<>
      <PageMeta title={pageTitle} description={`${pageTitle} в поселке Нагаево — специалисты и услуги в радиусе 50 км.`} canonical={isBeautySubRoute
      ...
      <div className={pageStyles.page}>
        <div className="container">
          ...
          {cat.slug !== 'beauty' && (<div className={styles.subcategoryGrid}>
              {cat.subcategories.map((sub) => (<Link key={sub.slug} to={searchPath(undefined, { category: cat.slug, subcategory: sub.slug })} className={styles.subcategoryLink}>
                  {sub.name}
                </Link>))}
            </div>)}
