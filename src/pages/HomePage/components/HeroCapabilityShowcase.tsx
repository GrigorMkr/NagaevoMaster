...
const HeroCapabilityShowcase = memo(function HeroCapabilityShowcase() {
  ...
  const card = (
    ...
    >
      ...
      {!lowPower && <span className={styles.flySheen} aria-hidden />}
      {!lowPower && <span className={styles.flyRipple} aria-hidden />}
      ...
      <div className={styles.flyInner}>
        ...
        <span className={styles.flyCta}>
          Открыть
          <ToolbarIcon name="chevronRight" accent={item.accent} motion={lowPower ? undefined : 'pulse'} />
        </span>
      </div>
    </Link>
  );
  ...
  return (
    <div className={styles.showcase} aria-live="polite">
      {!lowPower && <div className={styles.stageGlow} aria-hidden />}
