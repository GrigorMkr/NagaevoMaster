...
const FitListingsBounds = memo(function FitListingsBounds({ listings, enabled = true, }: FitListingsBoundsProps) {
    const map = useMap();
    useEffect(() => {
        if (!enabled)
            return;
        ...
        const bounds = L.latLngBounds(listings.map((listing) => [listing.location.lat, listing.location.lng]));
