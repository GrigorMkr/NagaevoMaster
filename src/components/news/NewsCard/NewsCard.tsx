...
const NewsCard = memo(function NewsCard({ item, variant = 'default' }: NewsCardProps) {
    const dateLabel = useMemo(() => format(new Date(item.publishedAt), 'd MMMM yyyy', { locale: ru }), [item.publishedAt]);
    const categoryLabel = NEWS_CATEGORY_LABELS[item.category];
    return (<article className={variant === 'compact' ? styles.cardCompact : styles.card}>
      <a href={item.sourceUrl} target="_blank" rel="noopener noreferrer" className={styles.imageLink}>
        <img src={item.imageUrl} alt="" className={styles.image} loading="lazy" onError={(e) => {
