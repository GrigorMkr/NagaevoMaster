/**
 * Применяет deploy/oauth.env на VPS и перезапускает API.
 *
 *   npm run vps:oauth
 */
...
const vars = loadEnv(oauthPath);
if (!vars) {
  console.error('Создайте deploy/oauth.env из deploy/oauth.env.example');
  process.exit(1);
}

const hasGoogle = vars.GOOGLE_CLIENT_ID && vars.GOOGLE_CLIENT_SECRET;
const hasVk = vars.VK_CLIENT_ID && vars.VK_CLIENT_SECRET;
if (!hasGoogle && !hasVk) {
  console.error('Заполните GOOGLE_* и/или VK_* в deploy/oauth.env');
  process.exit(1);
}
...
OAUTH_FILE=/tmp/oauth-merge.env
touch "$ENV_FILE"
...
echo "OAuth env applied."
`;

const oauthBody = KEYS.filter((k) => vars[k]).map((k) => `${k}=${vars[k]}`).join('\n') + '\n';
    ...
    try {
      await new Promise((resolve, reject) => {
        conn.sftp(async (err, sftp) => {
          if (err) return reject(err);
          try {
            await upload(sftp, '/tmp/oauth-merge.env', oauthBody);
            await upload(sftp, '/tmp/apply-oauth.sh', mergeScript, 0o755);
            resolve();
          } catch (e) {
            reject(e);
          }
      ...
      await run(conn, 'bash /tmp/apply-oauth.sh');
      conn.end();
